...
class TDModel(nn.Module):
    def __init__(self, encoder = config.encoder, decoder = config.decoder, use_pretrained_model = True, basemodel_requires_grad = True):
        ...
        # EfficientNet
            ...
            self.base_model = EfficientNet(arctype = 'b0', pretrained = use_pretrained_model, requires_grad = basemodel_requires_grad)
            self.pool_channels = [320, 192]
            self.table_decoder = ENDecoder(self.pool_channels, self.kernels, self.strides)
            ...
            self.base_model = EfficientNet(arctype = 'b1', pretrained = use_pretrained_model, requires_grad = basemodel_requires_grad)
            self.pool_channels = [320, 192]
            self.table_decoder = ENDecoder(self.pool_channels, self.kernels, self.strides)
            ...
            self.base_model = EfficientNet(arctype = 'b2', pretrained = use_pretrained_model, requires_grad = basemodel_requires_grad)
            ...
            self.base_model = EfficientNet(arctype = 'b3', pretrained = use_pretrained_model, requires_grad = basemodel_requires_grad)
            ...
            self.base_model = EfficientNet(arctype = 'b4', pretrained = use_pretrained_model, requires_grad = basemodel_requires_grad)
        ...
        if isinstance(self.base_model, ConvNext):
            self.conv6 = nn.Sequential(
                nn.Conv2d(in_channels = self.in_channels, out_channels = 512, kernel_size=(1,1)),
            ...
            self.conv6 = nn.Sequential(
                nn.Conv2d(in_channels = self.in_channels, out_channels = 1024, kernel_size=(1,1)),
                nn.ReLU(inplace=True),
                nn.Dropout(0.7),
