...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_dir", help = "Path to complete data's locate.csv file. (Applies train/test split.)", required = False)
    parser.add_argument("--test_dir", help = "Path to test data's locate.csv file. ", required = False)
    parser.add_argument("--no_csv", type = bool, help = "Do not write the CSV file.", required = False)
    parser.add_argument("--threshold", type = float, help = "Confidence score for calculating metrics. (Default = 0.6)", required = False)
