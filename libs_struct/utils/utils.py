    ...
    if isinstance(img_list, list):
        ...
        for img in img_list:
            if img.shape[1] < img_v or img.shape[0] < img_h:
                img_res = cv2.resize(
                    img, (img_v, img_h), interpolation=cv2.INTER_NEAREST
                )
        ...
        raise NotImplementedError(
            "To merge images img_list should be a list of cv2 images."
        )
...
def color_fix(imgA, imgB):
    ...
    if hA < hB and wA < wB:
        scaling = True
        imgB_ds = cv2.resize(imgB, dsize=(wA, hA), interpolation=cv2.INTER_CUBIC)
    else:
        imgB_ds = imgB
    ...
    if scaling:
        blurred = cv2.resize(blurred, dsize=(wB, hB), interpolation=cv2.INTER_CUBIC)
    rlt = blurred + imgB
...
def recompose_tensor(patches, height, width, step=None, scale=1):
    ...
    img_size = (
        patches.size(0),
        patches.size(1),
        max(full_height, patch_H),
        max(full_width, patch_W),
    )
    ...
    nrow, ncol = (
        1 + (img_size[-2] - patch_H) // step_int[0],
        1 + (img_size[-1] - patch_W) // step_int[1],
    )
    n_patches_height = (
        nrow + 1 if ((img_size[2] - patch_H) % step_int[0] != 0) else nrow
    )
...
def normalize_kernel2d(x: torch.Tensor) -> torch.Tensor:
    """Normalizes kernel."""
    if len(x.size()) < 2:
        raise TypeError("input should be at least 2D tensor. Got {}".format(x.size()))
...
def guided_filter(
    ...
    if not isinstance(box_kernel, torch.Tensor):
        ...
        if not ks:
            if r:
                ...
                raise ValueError(
                    "Either kernel size (ks) or radius (r) "
    ...
    if mode == "fast" or mode == "conv":
        mean_A = F.interpolate(
            A, (x_HR_shape[-2], x_HR_shape[-1]), mode="bilinear", align_corners=True
        )
        mean_b = F.interpolate(
            b, (x_HR_shape[-2], x_HR_shape[-1]), mode="bilinear", align_corners=True
        )
...
def getCoords(array):
    xmin = round(array[0][0])
    ymin = round(array[0][1])
    xmax = round(array[2][0])
    ymax = round(array[2][1])
