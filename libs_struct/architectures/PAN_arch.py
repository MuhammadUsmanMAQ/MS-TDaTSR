...
class SCPA(nn.Module):
    """SCPA is modified from SCNet (Jiang-Jiang Liu et al. Improving
        Convolutional Networks with Self-Calibrated Convolutions. In CVPR, 2020)
...
class PAN(nn.Module):
    """
    Efficient Image Super-Resolution Using Pixel Attention, in ECCV
    Workshop, 2020.
    ...
    def __init__(
        ...
        for i in range(n_upscale):
            if i < 1:
                if self.scale == 3:
                    upsampler.append(
                        pa_upconv_block(nf, unf, 3, 1, 1, self.ups_inter_mode, 3)
                    )
                else:
                    upsampler.append(
                        pa_upconv_block(nf, unf, 3, 1, 1, self.ups_inter_mode)
                    )
            else:
                upsampler.append(
                    pa_upconv_block(unf, unf, 3, 1, 1, self.ups_inter_mode)
                )
    ...
    def forward(self, x):
        ...
        if self.scale == 2 or self.scale == 3:
            fea = self.upconv1(F.interpolate(
                fea, scale_factor=self.scale, mode=self.ups_inter_mode, align_corners=True))
            fea = self.lrelu(self.att1(fea))
            fea = self.lrelu(self.HRconv1(fea))
        elif self.scale == 4:
            fea = self.upconv1(F.interpolate(
                fea, scale_factor=2, mode=self.ups_inter_mode, align_corners=True))
            fea = self.lrelu(self.att1(fea))
            fea = self.lrelu(self.HRconv1(fea))
            fea = self.upconv2(F.interpolate(
                fea, scale_factor=2, mode=self.ups_inter_mode, align_corners=True))
